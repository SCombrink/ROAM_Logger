...
export default function App() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setStatus("Launching browser in the background");
  ...
  useEffect(() => {
    if (isSubmitting && status === "Launching browser in the background") {
      const timer = setTimeout(() => {
        setStatus("Completing ROAM form");
      }, 17000); // Transitions after initial launch and tab waits
  ...
  return (
    <div style={{ backgroundColor: colors.bg, color: colors.text, fontFamily: "'Source Sans Pro', Arial, sans-serif", padding: "16px", minHeight: "100vh", userSelect: "none" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
          ...
          <button 
            ...
            style={{ ...btnStyle, backgroundColor: colors.surface, border: `1px solid ${colors.border}` }}
          >
            Send Feedback
          </button>
        </div>
      ...
      {/* Secret Debug Menu */}
      {showDebugMenu && (
        <div style={{ marginBottom: "16px", padding: "10px", backgroundColor: "#FFFBE6", border: `1px solid #FFE58F`, borderRadius: "8px", display: "flex", flexDirection: "column", gap: "10px" }}>
          <div style={{ fontSize: "12px", fontWeight: "bold", color: "#856404" }}>DEBUG SETTINGS</div>
          ...
          <button 
            ...
            style={{ ...btnStyle, backgroundColor: "#FFF", borderColor: colors.orange, color: colors.orange, width: "fit-content" }}
          >
            Clear API Key
          </button>
        </div>
        ...
        <div>
          <button 
            ...
            style={{ 
              padding: "12px 20px", 
              width: "100%",
              backgroundColor: isSubmitting ? colors.orange : (isFormValid() ? colors.primary : colors.border), 
              color: "#FFFFFF", 
              border: "none", 
              borderRadius: "8px", 
              fontWeight: "bold", 
              fontSize: "14px", 
              cursor: (isSubmitting || isFormValid()) ? "pointer" : "not-allowed", 
              marginTop: "10px" 
            }}
          >
